...
# --- ИНТЕРФЕЙСЫ ПРЕДПРОСМОТРА (КАК НА СКРИНШОТАХ) ---
...
async def prepare_game(call: types.CallbackQuery):
    ...
    if game == "dice":
        ...
        kb = [[InlineKeyboardButton(text="🏀 Попал - x1.6", callback_data="bet_basketball_goal")],
        ...
        kb = [[InlineKeyboardButton(text="🔴 Красное (x1.94)", callback_data="bet_darts_red"), InlineKeyboardButton(text="⚪ Белое (x2.9)", callback_data="bet_darts_white")],
              [InlineKeyboardButton(text="🎯 Центр (x5.8)", callback_data="bet_darts_center"), InlineKeyboardButton(text="😟 Мимо (x5.8)", callback_data="bet_darts_miss")]]
    elif game == "crash":
...
async def play_game(call: types.CallbackQuery):
    ...
    # Расширенная логика проверки
    if game_type == "dice":
        if choice.isdigit(): win, coef = (val == int(choice)), 5.8
        elif choice == "even": win, coef = (val % 2 == 0), 1.94
        elif choice == "odd": win, coef = (val % 2 != 0), 1.94
        elif choice == "v3": win, coef = (val == 3), 5.8
        ...
        if choice.isdigit(): win, coef = (val == int(choice)), 5.8
        else: win, coef = (choice == "strike" and val == 6) or (choice == "miss" and val == 1), 5.8
...
async def start_crash(call: types.CallbackQuery):
    ...
    for _ in range(50):
        ...
        if current_coef >= crash_point:
            ...
            await msg.edit_text(f"💥 **БОМБА! x{crash_point}**\n❌ Проигрыш: {user_data['bet']} руб.", 
                                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔄 Еще", callback_data="prep_crash")]]))
            return
        ...
        try: await msg.edit_text(f"🚀 Полет (Бомб нет): **x{current_coef}**", reply_markup=kb)
        except: pass
