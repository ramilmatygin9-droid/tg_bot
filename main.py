...
async def fast_menu(call: types.CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚀 Краш", callback_data="prep_crash")],[InlineKeyboardButton(text="◀️ назад", callback_data="to_main")]])
    await call.message.edit_text("🚀 **Быстрые игры:**", reply_markup=kb, parse_mode="Markdown")

@dp.callback_query(F.data == "prep_crash")
async def prep_crash(call: types.CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🛫 Взлёт!", callback_data="start_crash")],[InlineKeyboardButton(text="◀️ назад", callback_data="fast")]])
    await call.message.edit_text(f"🚀 **Краш**\n💸 Ставка: {user_data['bet']} руб.", reply_markup=kb)
...
async def start_crash(call: types.CallbackQuery):
    ...
    for _ in range(50):
        ...
        if current_coef >= crash_point:
            ...
            await msg.edit_text(f"💥 **ВЗРЫВ! x{crash_point}**", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔄 Еще", callback_data="prep_crash")]]))
            return
