...
# БАЗА ДАННЫХ ПОЛЬЗОВАТЕЛЕЙ (с расширенными данными)
users_db = {}
game_sessions = {} 
DOTS = "· · · · · · · · · · · · · · · · ·"
...
def main_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="🕹 Web Game", web_app=WebAppInfo(url="https://ramilmatygin9-droid.github.io/GLITCH-WIN/"))
        ],
        [
...
@dp.callback_query(F.data == "prep_wheel")
async def prepare_wheel(call: types.CallbackQuery):
    user = get_user(call.from_user.id)
    text = (f"🎡 **РУЛЕТКА**\n{DOTS}\n"
            f"💰 Ставка: {user['bet']} m¢\n\n"
            f"Выбери цвет и удвой ставку!")
...
@dp.callback_query(F.data == "prep_rps")
async def prepare_rps(call: types.CallbackQuery):
    user = get_user(call.from_user.id)
    text = (f"🪨✂️📄 **КАМЕНЬ-НОЖНИЦЫ-БУМАГА**\n{DOTS}\n"
            f"💰 Ставка: {user['bet']} m¢\n\n"
            f"Сыграй с ботом и выиграй x2!")
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def prepare_games(call: types.CallbackQuery):
    ...
    games_config = {
        ...
        "darts": {
            ...
            "text": f"🎯 *Дартс*\n{DOTS}\n💸 Ставка: {bet} m¢\n\nКуда попадёт дротик?",
            "buttons": [
                [InlineKeyboardButton(text="🔴 КРАСНОЕ (x1.94)", callback_data="play_dart_red"), InlineKeyboardButton(text="⚪ БЕЛОЕ (x2.9)", callback_data="play_dart_white")],
                [InlineKeyboardButton(text="🎯 ЦЕНТР (x5.8)", callback_data="play_dart_center"), InlineKeyboardButton(text="😟 МИМО (x5.8)", callback_data="play_dart_miss")]
            ]
        },
        "bowling": {
            "title": "🎳 БОУЛИНГ",
            ...
            "buttons": [
                ...
                [InlineKeyboardButton(text="🎳 СТРАЙК (x5.8)", callback_data="play_bowl_6"), InlineKeyboardButton(text="😟 МИМО (x5.8)", callback_data="play_bowl_0")]
            ]
        },
        "dice": {
            "title": "🎲 КУБИК",
    ...
    if game in games_config:
        ...
        kb = config["buttons"] + [[InlineKeyboardButton(text="◀️ Назад", callback_data="to_main")]]
        await call.message.edit_text(config["text"], reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    else:
        await call.answer("Игра в разработке")
