...
# --- ПОДГОТОВКА ИГР (МЕНЮ ИЗ СКРИНШОТОВ) ---
...
async def prepare_game(call: types.CallbackQuery):
    ...
    # Тексты и кнопки строго по вашим изображениям
    if game == "basketball":
        text = f"Рамиль\n🏀 **Баскетбол · выбери исход!**\n{DOTS}\n💸 Ставка: {user_data['bet']} mс"
        kb = [[InlineKeyboardButton(text="🏀 Гол - x1.6", callback_data="play_bask_anim")], 
              [InlineKeyboardButton(text="🗑 Мимо - x2.4", callback_data="play_bask_anim")], btn_back]
    elif game == "football":
        text = f"Рамиль\n⚽ **Футбол · выбери исход!**\n{DOTS}\n💸 Ставка: {user_data['bet']} mс"
        kb = [[InlineKeyboardButton(text="⚽ Гол - x1.6", callback_data="play_foot_anim")], 
              [InlineKeyboardButton(text="🥅 Мимо - x2.4", callback_data="play_foot_anim")], btn_back]
    elif game == "darts":
        text = f"Рамиль\n🎯 **Дартс · выбери исход!**\n{DOTS}\n💸 Ставка: {user_data['bet']} mс"
        ...
        kb = [[InlineKeyboardButton(text="1 кегля", callback_data="play_bowl_anim"), InlineKeyboardButton(text="3 кегли", callback_data="play_bowl_anim")],
              [InlineKeyboardButton(text="4 кегли", callback_data="play_bowl_anim"), InlineKeyboardButton(text="5 кегель", callback_data="play_bowl_anim")],
        ...
        kb = [[InlineKeyboardButton(text="1", callback_data="play_dice_anim"), InlineKeyboardButton(text="2", callback_data="play_dice_anim"), InlineKeyboardButton(text="3", callback_data="play_dice_anim")],
              [InlineKeyboardButton(text="4", callback_data="play_dice_anim"), InlineKeyboardButton(text="5", callback_data="play_dice_anim"), InlineKeyboardButton(text="6", callback_data="play_dice_anim")],
...
# --- ЕДИНЫЙ ДВИЖОК АНИМАЦИЙ ---
@dp.callback_query(F.data.startswith("play_"))
async def play_with_animation(call: types.CallbackQuery):
    ...
    # Списание ставки и запуск анимации
    user_data["balance"] -= user_data["bet"]
    msg = await call.message.answer_dice(emoji=emoji_map.get(game_type, "🎲"))
    
    await call.answer("Ставка принята! Ждем результат...")
    await asyncio.sleep(4) # Время анимации
    ...
    if game_type == "dice":
        if val >= 4: win, coef = True, 1.94
    elif game_type in ["foot", "bask"]:
        if val >= 3: win, coef = True, 1.6
    elif game_type in ["bowl", "dart"]:
        if val == 6: win, coef = True, 5.8
    elif game_type == "slots":
        if val in [1, 22, 43, 64]: win, coef = True, 10.0

    if win:
        profit = round(user_data["bet"] * coef, 2)
        user_data["balance"] += profit
        await call.message.answer(f"🎉 Победа! +{profit} mc\n💰 Баланс: {user_data['balance']} mc")
    else:
        await call.message.answer(f"📉 Проигрыш!\n💰 Баланс: {user_data['balance']} mc")
