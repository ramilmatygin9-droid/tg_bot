...
# База данных пользователей (в оперативной памяти)
users = {}
...
# ГЛАВНОЕ МЕНЮ (Точь-в-точь как на фото)
def main_kb():
...
def start(message):
    ...
    bot.send_message(message.chat.id, "💰 Добро пожаловать! Твой начальный баланс: 5000 золота.", reply_markup=main_kb())
...
def play(call):
    ...
    # Условия выигрыша
    if msg.dice.value >= 3: # Упростим: 3 и выше — победа
        u['balance'] += 1000
        bot.send_message(call.message.chat.id, f"✅ Ты выиграл! Баланс: {u['balance']}")
    else:
        bot.send_message(call.message.chat.id, f"❌ Проигрыш. Баланс: {u['balance']}")
...
def bomb_game(message):
    ...
    markup.add(*btns)
    bot.send_message(message.chat.id, "💣 Угадай пустую коробку (Ставка 500):", reply_markup=markup)
...
def bomb_logic(call):
    ...
    if random.choice([True, False]): # 50/50 шанс
        u['balance'] += 1000
        bot.edit_message_text(f"💎 Ура! Пусто! +1000 золота. Баланс: {u['balance']}", call.message.chat.id, call.message.id)
    else:
        u['balance'] -= 500
        bot.edit_message_text(f"💥 БУМ! Бомба! -500 золота. Баланс: {u['balance']}", call.message.chat.id, call.message.id)

@bot.message_handler(func=lambda m: m.text in ["⚙️ Настройки", "🆘 Помощь"])
def in_dev(message):
    bot.send_message(message.chat.id, "🛠 Этот раздел в разработке...")
