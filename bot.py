...
SHOP_PICKS = {
    ...
    6: {"name": "Обсидиановая кирка", "price": 300000, "mult": 20.0},
    7: {"name": "Изумрудная кирка", "price": 600000, "mult": 45.0},
    8: {"name": "Магматическая кирка", "price": 1200000, "mult": 100.0},
    9: {"name": "Кирка Бесконечности", "price": 5000000, "mult": 250.0},
    10: {"name": "👑 КИРКА СОЗДАТЕЛЯ", "price": 15000000, "mult": 1000.0}
}
...
async def main_start(message: types.Message):
    ...
    start_text = (
        ...
        f"🔹 <b>Команда:</b> /mine — копать.\n"
        f"🔹 <b>Кейсы:</b> /cases — испытать удачу.\n"
        f"🔹 <b>Улучшения:</b> /shop — новые кирки.\n"
    )
...
async def cases_menu(message: types.Message):
    ...
    for cid, info in CASES_DATA.items():
        ...
        kb.append([
            ...
            InlineKeyboardButton(text="Открыть", callback_data=f"opencase_{cid}")
        ])
...
async def open_case(c: types.CallbackQuery):
    cid = c.data.split("_")[1]
    p = get_player(c.from_user.id)
    if p["cases"].get(cid, 0) <= 0: return await c.answer("У вас нет этого кейса!", show_alert=True)
    
    p["cases"][cid] -= 1
    item = random.choices(CASES_DATA[cid]["items"], weights=[i["chance"] for i in CASES_DATA[cid]["items"]])[0]
    
    if item["type"] == "money":
        win = random.randint(item["min"], item["max"])
        db_query("UPDATE players SET balance = balance + ?, cases = ? WHERE user_id = ?", (win, json.dumps(p["cases"]), c.from_user.id), commit=True)
        res = f"💰 Выпало: <b>{win} монет!</b>"
    else:
        p["crystals"][item["key"]] = p["crystals"].get(item["key"], 0) + 1
        ...
        res = f"💎 Выпал кристалл: <b>{CRYSTALS_DATA[item['key']]['name']}</b>!"
    
    await c.message.answer(f"📦 <b>Открытие {CASES_DATA[cid]['name']}...</b>\n\n{res}", parse_mode="HTML")
    await cases_menu(c.message)
    await c.message.delete()
...
async def crystals_cmd(message: types.Message):
    ...
    for key, count in p["crystals"].items():
        if count > 0: text += f"• {CRYSTALS_DATA[key]['name']}: <b>{count}</b> шт.\n"
    await message.answer(text if "шт" in text else "Пусто.", parse_mode="HTML")
...
async def promo_cmd(message: types.Message, command: CommandObject):
    ...
    if promo:
        p = get_player(message.from_user.id)
        if code in p["used_promos"]: return await message.reply("Уже был!")
...
async def main():
    ...
    await main_bot.set_my_commands([
        ...
        BotCommand(command="/crystals", description="Коллекция"),
