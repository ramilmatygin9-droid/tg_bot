...
async def cmd_inventory(message: types.Message):
    ...
    text = (f"🎒 <b>Инвентарь:</b>\n"
            f"⛏ Кирка: <b>{pick}</b>\n\n"
            f"💎 Обычные: {p['common']} шт.\n"
            f"💎 Полуредкие: {p['uncommon']} шт.\n"
            f"💎 Редкие: {p['rare']} шт.\n\n"
            f"💰 Стоимость: <b>{total:,}</b>")
    
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💰 Продать всё", callback_data="sale_action")]])
...
async def cmd_sale(message: types.Message):
    ...
    total = (p['common']*1000 + p['uncommon']*5000 + p['rare']*15000)
    if total == 0:
        return await message.answer("У тебя пусто!")
    ...
    await message.answer(f"🤓 Продано за <b>{total:,}</b> монет!")
...
async def cmd_bonus(message: types.Message):
    ...
    if now - p['last_bonus'] < 86400:
        return await message.answer("⏳ Бонус раз в 24 часа!")
    ...
    await message.answer(f"🎁 Получен бонус <b>{reward}</b> монет!", parse_mode="HTML")
...
async def callbacks(c: types.CallbackQuery):
    if c.data == "mine_action": await cmd_mine(c.message)
        ...
        await c.message.answer("🛒 Магазин:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
        ...
        if p['balance'] >= SHOP_PICKS[lvl]['price']:
            ...
            await c.answer(f"Куплено: {SHOP_PICKS[lvl]['name']}!")
        else:
